...
void HPF(int sem1, int sem2, int algorithm, ProcessData* shmaddr);
void SRTN(int sem1, int sem2, int algorithm, ProcessData* shmaddr);
void RR(int sem1, int sem2, int algorithm, ProcessData* shmaddr, int quantum);
void OutputFiles(Queue* PCB)
{
...
int main(int argc, char* argv[])
{
    ...
    if (algorithm == 3) {
        HPF(sem1, sem2, algorithm, shmaddr);
    }

    if (algorithm == 4) {
        SRTN(sem1, sem2, algorithm, shmaddr);
    }
...
void SJF(int sem1, int sem2, int algorithm, ProcessData* shmaddr) {
    ...
    while (1) {
        ...
        if (runningProcess.remainingTime == 0 && peekList(readyQueue, &firstInQueue)) {
            ...
            runningProcess.startTime = getClk() - 1;
            runningProcess.status = started;
            runningProcess.logTime = getClk() - 1;
            runningProcess.waitingTime = runningProcess.startTime - runningProcess.arrivalTime;
            enqueue(PCB, runningProcess);
            //printf("my data pid = %d, remainingtime =%d\n",runningProcess.pid,runningProcess.arrivalTime);
        }
...
void RR(int sem1, int sem2, int algorithm, ProcessData* shmaddr, int quantum)
{
    ...
    readyQueue->front = 0;
    readyQueue->max = 5;
    readyQueue->Arr = (ProcessData*)malloc((readyQueue->max) * sizeof(ProcessData));
    char buf[50];
    getcwd(buf, sizeof(buf));
    ProcessData runningProcess;
    runningProcess.remainingTime = 0;
    ...
    while (1)
    {
        ...
        if ((!isEmptyArr(readyQueue)) && (currentTime - timeCounter == quantum || runningProcess.remainingTime == 0))
        {
            ...
            if (runningProcess.remainingTime > 0)
            {
                ...
                enqueueArr(readyQueue, runningProcess);
                runningProcess.logTime = getClk();
                enqueue(PCB, runningProcess);
            }
            ...
            {
                runningProcess.status = resumed;

            }
            runningProcess.waitingTime = getClk() - runningProcess.arrivalTime - (runningProcess.runningTime - runningProcess.remainingTime);
            runningProcess.logTime = getClk();
            enqueue(PCB, runningProcess);
...
void SRTN(int sem1, int sem2, int algorithm, ProcessData* shmaddr) {
    ...
    int prevTime = 0;
    int currentTime;
    runningProcess.remainingTime = 10000;
    runningProcess.id = -1;
    //prev.id = -1;
    ...
    while (1) {
                ...
                if (temp.id != -1) {
                    if (temp.id != prev.id && temp.arrivalTime > 0) {
                        ...
                        if (processPid != 0) {
                            ...
                            enqueueList(readyQueue, temp, temp.remainingTime);

                            //Peek and see if the front process is the same as the current if not 
                            //Print Procees stopped at getclk;
                            // enque the current running proccess if not equal NULL
                            printf("%d\n", temp.pid);
                        }
                        if (processPid == 0) {
                            //temp.pid = getpid();
                            ...
                            getcwd(buf, sizeof(buf));
                            strcat(buf, "/process.out");
                            execl(buf, "process.out", "process.out", NULL);
                        }
                        int sendRunningTime;
                        message.mtype = processPid;
                        message.mRemainingTime = temp.runningTime;
        ...
        ProcessData firstInQueue;
        if (peekList(readyQueue, &firstInQueue))
        {

            if ((firstInQueue.remainingTime < runningProcess.remainingTime) || runningProcess.remainingTime == 0) {
                if (runningProcess.id != -1 && runningProcess.remainingTime > 0)
                {
                    runningProcess.status = stopped;
                    printf("Stopeedddddddddddddddddddddd id = %d, at %d\n", runningProcess.id, getClk());
                    runningProcess.logTime = getClk() - 1;
                    enqueue(PCB, runningProcess);
                    enqueueList(readyQueue, runningProcess, runningProcess.remainingTime);
                }

                dequeueList(readyQueue, &runningProcess);
                if (runningProcess.remainingTime == runningProcess.runningTime)
                {
                    printf("Starteddddddddddddddddddddddddddd id = %d, at %d\n", runningProcess.id, getClk());
                    runningProcess.startTime = getClk() - 1;
                    runningProcess.status = started;
                    runningProcess.logTime = getClk() - 1;
                    runningProcess.waitingTime = runningProcess.startTime - runningProcess.arrivalTime;
                    ...
                    runningProcess.status = resumed;
                    printf("resumeddddddddddddddddddd\n");
                    runningProcess.logTime = getClk() - 1;
                    enqueue(PCB, runningProcess);
                }
            }
...
void HPF(int sem1, int sem2, int algorithm, ProcessData* shmaddr) {
    ...
    while (1) {
                ...
                if (temp.id != -1) {
                    if (temp.id != prev.id && temp.arrivalTime > 0) {
                        ...
                        if (processPid != 0) {
                            ...
                            enqueueList(readyQueue, temp, temp.priority);
        ...
        if (peekList(readyQueue, &firstInQueue))
        {
            ...
            if ((firstInQueue.priority < runningProcess.priority) || runningProcess.remainingTime == 0) {
                if (runningProcess.id != -1 && runningProcess.remainingTime > 0)
                {
                    runningProcess.status = stopped;
                    printf("Stopeedddddddddddddddddddddd id = %d, at %d\n", runningProcess.id, getClk());
                    runningProcess.logTime = getClk() - 1;
                    enqueue(PCB, runningProcess);
                    enqueueList(readyQueue, runningProcess, runningProcess.priority);
                }

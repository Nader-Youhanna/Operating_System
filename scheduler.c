...
void SRTN(int sem1, int sem2, int algorithm, ProcessData* shmaddr) {
    ...
    while (1) {
                ...
                if (temp.id != -1) {
                    if (temp.id != prev.id && temp.arrivalTime > 0) {
                        ...
                        if (processPid != 0) {
                            ...
                            if (runningProcess.remainingTime != 0 && runningProcess.runningTime != runningProcess.remainingTime)
                            {
        ...
        {
            if (firstInQueue.id != runningProcess.id) {
                runningProcess.status = stopped;
                runningProcess.logTime = getClk();
                enqueue(PCB, runningProcess);
            }
            dequeueList(readyQueue, &runningProcess);
            if (firstInQueue.remainingTime == firstInQueue.runningTime) {
                runningProcess.startTime = getClk() - 1;
                runningProcess.status = started;
            ...
            else if (firstInQueue.status == stopped) {
        ...
        /*         peekList(readyQueue, &firstInQueue);
                if ( firstInQueue.id != runningProcess.id && runningProcess.status!=finished) {
                    runningProcess.status = stopped;
                    runningProcess.logTime = getClk();
                    enqueue( PCB, runningProcess);

                    dequeueList(readyQueue, &runningProcess);
                    if(runningProcess.status == stopped)
                ...
                if (temp.id != -1) {
                    if (temp.id != prev.id && temp.arrivalTime > 0) {
                        ...
                        if (processPid != 0) {
                            ...
                            // re queue the previous process in the PQ if it didn't finish in the previous cycle
                            if(runningProcess.status != finished  && runningProcess.remainingTime != runningProcess.runningTime )
                            {
